...
/**
 * Plugin to do AI keywords suggestions.
 */
#[AiCKEditor(
  id: 'ai_ckeditor_keywords_suggestion',
  label: new TranslatableMarkup('Keywords suggestion'),
  description: new TranslatableMarkup('Suggesting keywords for the text'),
)]
final class KeywordsSuggestion extends AiCKEditorPluginBase {
  ...
  public function buildCkEditorModalForm(array $form, FormStateInterface $form_state) {
    ...
    if (empty($storage['selected_text'])) {
      return [
        '#markup' => '<p>' . $this->t('You must select some text for which you want to suggest keywords for.') . '</p>',
      ];
    }
    ...
    $form['page_keywords'] = [
      '#type' => 'checkbox',
      '#title' => $this->t('Add to page keywords'),
    ];
    ...
    $form['response_text'] = [
      '#type' => 'text_format',
      '#title' => $this->t('The suggested keywords'),
    ...
    $form['actions']['generate']['#value'] = $this->t('Suggest keywords');
  ...
  public function ajaxGenerate(array &$form, FormStateInterface $form_state) {
    ...
    try {
      $prompt = 'Generate 10 keywords that I can for the following text :\r\n"' . $values["plugin_config"]["selected_text"];
